...
const vec3 camerainit(0.0, 0.0, 5.0); // Initial camera position, also for resets
const vec3 upinit(0.0, 1.0, 0.0); // Initial up position, also for resets
const int arrowForceInit = 5; //Initial step amount for camera movement, also for resets
...
void loadTexture(char *start, char *end)
{
	...
	for (int ii = 1; ii <= no_of_images; ii++)
	{
		string name(start);
		int namesupport = 5;
		int x = ii;
		...
		texture[ii] = makeTexture(writable);
		delete[] writable;
	}	
}	
...
int main(int argc, char* argv[]) {
	...
	//load texture call
	loadTexture("tame001.png","tame001.png");
